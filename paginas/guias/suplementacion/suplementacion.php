...
if (!ControlSesion::sesion_iniciada()) {
  ...
  $titulo = "Guía de suplementación ".$nombreEmpresa;
  $tipoGuia = "guia-suplementacion";
  $pdf = DIRECTORIO_RAIZ."/pdf/".$tipoGuia."/".$usuario->obtener_email()."-".$tipoGuia;

  if (!file_exists($pdf)) {
    Redireccion::redirigir(RUTA_GUIA_SUPLEMENTACION_NO_DISPONIBLE);
  }else{    
    //el pdf si existe
  }
}
...
include_once 'seccion/guias/suplementacion/portada.inc.php';
include_once 'seccion/guias/suplementacion/recomendaciones.inc.php';
...
<script type="text/javascript">
  document.addEventListener("adobe_dc_view_sdk.ready", function(){
    ...
    adobeDCView.previewFile({
      content: { location: { url: "<?php echo RUTA_GUIA_SUPLEMENTACION_PDF ."/".$usuario->obtener_email()."-".$tipoGuia; ?>" } },
