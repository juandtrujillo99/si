...
class RepositorioEdicionDatos{
	...
	public static function generar_peticion($conexion, $id_usuario, $url_secreta){
		...
		if (isset($conexion)) {
			try{
				$sql = "INSERT INTO editar_datos(usuario_id, url_secreta, fecha) VALUES (:usuario_id, :url_secreta, NOW())";
	...
	public static function url_secreta_existe($conexion, $url_secreta){
		...
		if (isset($conexion)) {
			try{
				$sql = "SELECT * FROM editar_datos WHERE url_secreta = :url_secreta";
	...
	public static function obtener_id_usuario_mediante_url_secreta($conexion, $url_secreta){
		...
		if (isset($conexion)) {
			try{
				include_once 'EdicionDatos.inc.php';
